...
public class Main {
	...
	static {
		try (RandomAccessFile rb = new RandomAccessFile("files/student.idx", "r");) {
			loadIndex(rb, 0);
		} catch (IOException e) {
	...
	public static void main(String[] args) {
		...
		// BuildStudentData.build("files/student", true);

		try (RandomAccessFile ra = new RandomAccessFile("files/student.dat", "r")) {
			// loadIndex(ra, 0);
