...
public class Theatre {
    ...
    private Boolean validate(int count, char first, char last, int min, int max) {
        ...
        if (!result) {
            System.out.printf(
                    "Invalid! %1$d seats between " +
                            "%2$c[%3$d-%4d]-%5$c[%3$d-%4$d] Try again",
                    count, first, min, max, last
            );
            System.out.printf(
                    ": Seat must be between %s and %s%n",
                    seats.first().seatNum, seats.last().seatNum
            );
        }
        return result;
   ...
   public Set<Seat> reservedSeats(int count, char minRow, char maxRow,
